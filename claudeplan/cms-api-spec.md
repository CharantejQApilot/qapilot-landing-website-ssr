...
```json
{
  ...
  "_optional_cover": "cover_media_id and cover_image_url may be null or omitted entirely. The CMS must accept posts with no cover image — reviewer will upload one in the admin UI later.",
...
## Admin UI requirement — "Publish & Index" button

On the draft edit page in your CMS admin, add two action buttons that fire when the
post's current `tier == "draft"`:

1. **Publish & Index** (primary, prominent — this is the normal flow)
   - On click: server calls `PATCH /api/posts/{id}` internally with
     `{"tier": "index_worthy", "status": "published"}`
   - On success: show a toast "Published. URL: /resources/<cluster>/<slug> — added to
     sitemap" and redirect to the live URL.
   - Disabled (with tooltip "Article failed the quality gate — review required") if
     `quality_checks.overall_recommendation == "DISCARD"`.
   - Disabled (with tooltip "Pick a Topic Cluster first") if `topic_cluster` is empty.

2. **Save as Supporting** (secondary, small — only for glossary-style pages)
   - On click: server calls `PATCH /api/posts/{id}` with
     `{"tier": "supporting", "status": "published"}`
   - On success: toast "Saved as supporting — at /resources/<cluster>/<slug>, noindex,follow."

Both buttons hit the same `PATCH` endpoint documented below. The buttons disappear (or
are replaced with "Already Published" / a "Revert to Draft" action) once `tier != "draft"`.

Also recommended in the draft edit UI: render the `quality_checks` JSON as a small panel
showing originality / usefulness / product-relevance scores, the `information_gain`
bullets, and a "Claims to verify" checklist that lets the editor tick off each fact
before clicking Publish & Index.
