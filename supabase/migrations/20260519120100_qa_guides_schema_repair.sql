-- Repair partial qa_guides table (e.g. created without tier/status).
-- Safe to re-run. Run in SQL Editor if 20260519120000 failed partway through.
...
-- Legacy tables may lack slug/title. Drop empty broken shells only (before CREATE).
DO $$
DECLARE
  row_count bigint;
  has_slug boolean;
...
-- Add columns missing from a partial / legacy qa_guides table.
...
BEGIN
  IF EXISTS (
    ...
    UPDATE public.qa_guides SET title = 'Untitled guide' WHERE title IS NULL OR btrim(title) = '';
  END IF;
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'qa_guides' AND column_name = 'slug'
  ...
  IF EXISTS (
    ...
    UPDATE public.qa_guides SET topic_cluster = 'flutter-testing' WHERE topic_cluster IS NULL OR btrim(topic_cluster) = '';
  END IF;
END $$;

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'qa_guides' AND column_name = 'title') THEN
    ALTER TABLE public.qa_guides ALTER COLUMN title SET NOT NULL;
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'qa_guides' AND column_name = 'slug') THEN
    ALTER TABLE public.qa_guides ALTER COLUMN slug SET NOT NULL;
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'qa_guides' AND column_name = 'topic_cluster') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'qa_guides_topic_cluster_fkey') THEN
...
WHERE (url_path IS NULL OR btrim(url_path) = '') AND slug IS NOT NULL;
