...
export function PlatformTestingSolutionSection({ content }: { content: PlatformTestingContent }) {
  return (
    <section
      className="section-edge relative w-full overflow-hidden border-t border-border/40 bg-dot-pattern-subtle"
      aria-labelledby={`${content.slug}-solution-heading`}
    >
      <div
        className="pointer-events-none absolute inset-0 bg-gradient-to-b from-muted/[0.35] via-transparent to-transparent"
        aria-hidden
      />
      <div className="section-full relative z-10 py-14 md:py-20 2xl:py-24">
        <MarketingSectionHeader
          id={`${content.slug}-solution-heading`}
          title={
            <>
              {content.solutionHeadingBefore}{" "}
              <span className="text-primary">{content.solutionHeadingAccent}</span> Complexity
            </>
          }
        ...
        <ul className="grid gap-5 sm:grid-cols-2 lg:gap-6">
          {content.solutionCards.map((card) => (
            <li
              key={card.title}
              className={cn(
            ...
            >
              ...
              <div className="relative flex flex-col gap-4 sm:flex-row sm:gap-5">
                ...
                <div className="min-w-0 flex-1 pt-0.5">
                  ...
                  <p className="mt-2 text-sm text-muted-foreground md:text-base">{card.line}</p>
                </div>
              </div>
            </li>
          ))}
