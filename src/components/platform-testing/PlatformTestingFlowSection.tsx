...
import { PLATFORM_FLOW_BASE_STEPS, type PlatformTestingContent } from "@/lib/platform-testing";

export function PlatformTestingFlowSection({ content }: { content: PlatformTestingContent }) {
  const steps = PLATFORM_FLOW_BASE_STEPS.map((step) =>
    step.label === "Test" ? { ...step, line: content.flowTestStepLine } : step,
  );

  return (
    <section
      className="section-edge relative w-full overflow-hidden border-t border-border/50 bg-muted/[0.2]"
      aria-labelledby={`${content.slug}-flow-heading`}
    >
      <div className="section-full relative z-10 py-14 md:py-20 2xl:py-24">
        <MarketingSectionHeader
          id={`${content.slug}-flow-heading`}
          title={
            <>
          ...
          {steps.map((step, index) => (
            <li key={step.label} className="group flex list-none gap-4">
              ...
              <div className="min-w-0 pb-10 pt-1">
                <h3 className="font-heading text-base font-semibold text-foreground">{step.label}</h3>
        ...
        <div className="relative hidden lg:block">
          ...
          <ol className="relative grid grid-cols-5 gap-4">
            {steps.map((step) => (
              <li key={step.label} className="group flex list-none flex-col items-center text-center">
                <span
