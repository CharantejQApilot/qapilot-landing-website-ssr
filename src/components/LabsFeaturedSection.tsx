...
/** Labs featured blogs — server-rendered for SEO. */
export default async function LabsFeaturedSection() {
  const supabase = tryCreateServerSupabaseClient();
  if (!supabase) return null;
  const { data: posts, error } = await supabase
    .from("blogs")
    .select(
      "id, title, slug, excerpt, featured_image, author_name, published_date, youtube_url"
    )
    .eq("published", true)
      ...
      <div className="container mx-auto max-w-screen-xl px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="text-muted-foreground max-w-2xl mx-auto">
            Projects born from hackathons, side experiments, and internal needs.
        ...
        {!posts || posts.length === 0 ? (
          ...
          <div className="space-y-8">
            {(posts as LabsFeaturedPost[]).map((post, index) => {
              const isEven = index % 2 === 0;
              const imageSrc =
                post.featured_image ||
                (post.youtube_url
                  ? getYouTubeThumbnail(post.youtube_url)
                  : null) ||
                undefined;
              ...
              return (
                ...
                >
                  <div
                    className={`md:w-1/2 h-56 md:h-72 overflow-hidden ${!isEven ? "md:order-2" : ""}`}
                  >
                    {imageSrc ? (
                      <img
                        src={imageSrc}
                  ...
                  <div
                    className={`md:w-1/2 p-6 md:p-10 flex flex-col justify-center ${!isEven ? "md:order-1" : ""}`}
                  >
                    <h3 className="text-xl md:text-2xl font-bold text-foreground mb-3 group-hover:text-primary transition-colors line-clamp-2">
                      {post.title}
                    </h3>
                    {post.excerpt && (
                      <p className="text-sm md:text-base text-muted-foreground line-clamp-3 mb-5">
                        {post.excerpt}
                      </p>
                    )}
                    <div className="flex items-center gap-3 text-xs text-muted-foreground">
                      ...
                      {post.author_name && post.published_date && (
